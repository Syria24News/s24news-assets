...
(function () {
  ...
  /* معايرة على تقويم الجامع الأموي — 14 أيلول 2026:
     الفجر والشروق والعشاء متطابقة بلا إزاحة.
     الظهر والعصر: الفارق ±دقيقة كان ضجيج تقريب لا هامشاً حقيقياً — Aladhan يُرجع الوقت مقرَّباً
     سلفاً إلى أقرب دقيقة، فأي إزاحة ثابتة تُصيب يوماً وتُخطئ آخر حسب موقع اللحظة الفلكية
     الحقيقية من حدّ الدقيقة. لذا أُلغيت الإزاحة عنهما نهائياً، ويُقبل فارق ±دقيقة طبيعي أحياناً.
     المغرب +2: هامش احتياط حقيقي وثابت (أكبر من أي ضجيج تقريب مُحتمل)، فبقي كما هو.
     الترتيب: الإمساك,الفجر,الشروق,الظهر,العصر,المغرب,الغروب,العشاء,منتصف الليل */
  var TUNE   = '0,0,0,0,0,2,0,0,0';
  var CACHE_V = 4;                       // ارفعه كلما غيّرت TUNE أو METHOD ليُهمَل المخزَّن القديم
  ...
  var PAGE_CSS = [
    ...
    '.s24pt-scroll{overflow:auto;-webkit-overflow-scrolling:touch;max-height:70vh;border:1px solid rgba(128,128,128,.22);border-radius:8px}',
    '.s24pt-table{width:100%;min-width:520px;border-collapse:separate;border-spacing:0;font-size:14px}',
    ...
    '.s24pt-table td:first-child,.s24pt-table th:first-child{text-align:right;white-space:nowrap;position:sticky;right:0;background:var(--bg-card,#fff);will-change:transform}',
    '.s24pt-table thead th{position:sticky;top:0;background:#ecf6df;will-change:transform;z-index:2}',
    '.s24pt-table thead th:first-child{top:0;z-index:3;background:#ecf6df}',
    '[data-theme="dark"] .s24pt-table thead th{background:#2a341e}',
    '[data-theme="dark"] .s24pt-table thead th:first-child{background:#2a341e}',
    '.s24pt-table tbody td:first-child{z-index:1}',
    '.s24pt-table tr.is-today td{background:rgba(128,128,128,.14);font-weight:600}',
    '.s24pt-table tr.is-today td:first-child{background:#ececec}',
    '[data-theme="dark"] .s24pt-table tr.is-today td:first-child{background:#2c2c2c}',
