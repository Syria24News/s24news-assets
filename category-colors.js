/* ============================================================
   ...
   [تنظيف] تمت إزالة مفاتيح مكررة من خريطة الألوان (*L, *SW, *WN)
   وحذف قسم "النظام القديم" الذي كان يحوي فقط نسخة مكررة من *WN.
   [تنظيف] تمت إعادة ترتيب priorityList: حذف الأكواز الميتة
   (عناصر كانت مكررة في المستوى 6 ولن تُنفَّذ أبداً لأن المستوى 5
   يطابقها أولاً)، وحذف سطرين فارغين بلا وظيفة.
   [محدَّث] "نفط" أصبح له الآن متغيّر CSS مستقل خاص به
   (--sy-oil، مربوط بمتغيّر بلوجر cat.oil) بدل الاعتماد على
   لون "طاقة" — يتطلب هذا إضافة الـ Variable المقابلة في قالب
   بلوجر نفسه (خارج نطاق هذا الملف).
   [تنبيه لم يُحسَم]: "العملات والذهب" في priorityList ما زال
   بلا مفتاح مطابق تماماً في خريطة الألوان (الموجود هو
   "صرف العملات والذهب") — بانتظار تأكيد اللون المقصود.
   ============================================================ */
...
"العملات والذهب":"سعر صرف العملات والذهب",
    ...
    function applyUnifiedColors() {
        ...
        // [تنظيف] المستويات 1-3 محجوزة لعناوين مستقبلية (فارغة حالياً عمداً)
        const priorityList = [

            // 🔥 المستوى 1: عاجل وخطير (يغلب أي شيء) — محجوز، لا عناصر بعد

            // 📂 المستوى 2: وثائق ومناطق جغرافية محددة — محجوز، لا عناصر بعد

            // 🎯 المستوى 3: تخصصات دقيقة (نيش) — محجوز، لا عناصر بعد
            ...
            "الكرة السورية",
            "مجتمع",
            "العملات والذهب",       // ⚠ خريطة الألوان تستخدم "صرف العملات والذهب"
            ...
            // 🌍 المستوى 6: تصنيفات عامة وشاملة (الأضعف) — عناصر جديدة فقط
            "عالم",
            "سوريا",
            "*Sy"
        ];
